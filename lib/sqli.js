...
ConnectionPromise.prototype._checkState = function() {
  if (this._released) {
    ...
    throw new Error(msg);
  } else if (this._error) throw new Error('Connection is paused due to an error');
};
...
ConnectionPromise.prototype._runIfNext = function(stmt) {
  var shouldRun = 
      ...
      this._connection !== null && 
      this._queue[0] === stmt;
  //console.error((this._pendingStmt === null) + ',' + (this._connection !== null) + ',' + (this._queue[0] === stmt));
 ...
 */
ConnectionPromise.prototype.resume = function() {
