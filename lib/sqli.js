...
function ConnectionPromise(wrapper, resolver, releaser) {
  ...
  resolver.once('resolve', function(err, conn) {
    ...
    resolved = true; 
    callbacks();
    if (statementQueue[0] !== undefined)
      run(statementQueue[0]);
  });
  ...
  function run(stmt) {
    if (connection !== null) {
      wrapper.execute(connection, stmt[0], stmt[1], function(err, innerCursor) {
        if (statementQueue[0] !== undefined) {
          // schedule the next queued statement
          process.nextTick(function(){ run(statementQueue.shift()); });
        }
        if (err) lastError = err;
        var resolver = stmt[2];
  ...
  this.execute = function(sql, params) {
    ...
    // only execute if this is the first in queue and the connection is
    // resolved
    if (resolved && stmt === statementQueue[0]) run(stmt);
...
function ResultPromise(resolver) {
  ...
  function callbacks() {
    if (typeof callback === 'function' && cursor !== null) {
      callback();
    }
