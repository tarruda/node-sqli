...
ConnectionPromise.prototype._run = function(stmt) {
  ...
  if (this._error) {
    resolver.emit('error', this._error);
    this._schedule();
    return;
  }
  ...
  this._wrapper.execute(this._connection, stmt[0], stmt[1], 
    ...
    function(err) {
      if (err && self._error === null) self._error = err;
...
ConnectionPromise.prototype._runIfNext = function(stmt) {
  if (this._pendingStmt === null && this._connection !== null && 
      this._queue[0] === stmt)
...
*/
ConnectionPromise.prototype.execute = function(sql, params) {
  this._checkState();
  if (!params)
    params = undefined;
...
ConnectionPromise.prototype.commit = function() {
  this._checkState();
  var stmt = [this._wrapper.commit(), null, null];
  if (this._queue) {
    this._queue.push(stmt);
    this._runIfNext(stmt);
  }
...
ConnectionPromise.prototype.save = function(savepoint) {
  this._checkState();
  var stmt = [this._wrapper.save(savepoint), null, null];
  if (this._queue) {
    this._queue.push(stmt);
    this._runIfNext(stmt);
  }
...
ConnectionPromise.prototype.rollback = function(savepoint) {
  var stmt = [this._wrapper.rollback(savepoint), null, null];
  if (this._queue) {
    this._queue.push(stmt);
    this._runIfNext(stmt);
  }
